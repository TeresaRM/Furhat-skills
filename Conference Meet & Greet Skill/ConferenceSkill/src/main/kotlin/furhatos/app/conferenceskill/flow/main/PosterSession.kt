...
val PosterSession: State = state(Parent) {
    onEntry {
        ...
        furhat.say {
            random {
                + "Well, one last question FIX, I'm genuinely interested"
                + "Well, one last question FIX and I'll let you go"
            }
        }
    ...
    onResponse<NegativeRemark> {
        furhat.say {
            random {
                + "Thanks for the feedback FIX. Me and my team will remember that"
    ...
    onNoResponse {
        furhat.say("FIX are you okay? Don't you like this poster session?")
        furhat.listen()
    }
}